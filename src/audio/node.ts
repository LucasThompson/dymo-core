...
import { AMPLITUDE, REVERB, DELAY, FILTER, PAN, HEIGHT, DISTANCE } from '../globals/uris';

const POSITION_PARAMS = [PAN, HEIGHT, DISTANCE];
...
export class DymoNode {
	...
	constructor(dymoStore: DymoStore, dymoUri: string, audioContext: AudioContext, reverbSend, delaySend) {
	...
	private init(audioContext, reverbSend, delaySend) {
		...
		if (this.dymoStore.findParameterUri(this.dymoUri, PAN)
				|| this.dymoStore.findParameterUri(this.dymoUri, HEIGHT)
				|| this.dymoStore.findParameterUri(this.dymoUri, DISTANCE)) {
		...
		if (this.dymoStore.findParameterUri(this.dymoUri, FILTER)) {
	...
	protected addParameter(paramType, webAudioParam) {
		this.dymoStore.addParameterObserver(this.dymoUri, paramType, this);
		this.parameters[paramType] = webAudioParam;
		var paramValue = this.dymoStore.findParameterValue(this.dymoUri, paramType);
		if (paramValue != null) {
	...
	/** @private */
	private setParameter = function(paramType, value) {
		if (!isNaN(value)) {
	...
	removeAndDisconnect() {
		...
		for (var p in this.parameters) {
			this.dymoStore.removeParameterObserver(this.dymoUri, p, this);
		}
